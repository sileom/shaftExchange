...
//int move(std::string &robot_ip, Eigen::Vector3d final_point, Eigen::MatrixXd final_matrix, double tf, int seg);
    while(seg<=3){
      if(seg==1){
        esito = move(robot_ip,  Eigen::Vector3d(P(1,0), P(1,1),P(1,2)),Rf_vec[1], 0, tempo); 
        if(esito==-1)
          break;
    
      }else if(seg==2){    
        esito = move(robot_ip,  Eigen::Vector3d(P(2,0), P(2,1),P(2,2)),Rf_vec[2], 0, tempo); 
        ...
        esito = move(robot_ip,  Eigen::Vector3d(P(3,0), P(3,1),P(3,2)),Rf_vec[3], 0, tempo);   
